...
[Hotwire/ERBにReactを埋め込むことは簡単にできます](/other_libraries/using_with_react)。これは昔から行われているやり方で、Reactを発明したFacebook自身でも当初はこの使い方をしていました。

この方法を使うとUI/UXが劣るのではないかという考えがあります。React SPAにしてしまった方が良くなるのではないかという考えです。しかしこれは誤りです。UI/UXに世界一うるさいAppleのウェブサイトでも、ERBにReactを埋め込んでいます。問題はあくまでも実装方法にあり、UI/UXが劣るのは、劣るような実装をしているからです。

[Appleのウェブサイトなどで使用されている方法](/introduction/using_with_react)を別途解説していますので、ご覧ください。
