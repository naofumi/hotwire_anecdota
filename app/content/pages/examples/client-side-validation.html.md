...
Ruby on Railsは双方の役割を果たす機能を従来から用意してきました。しかしブラウザネイティブのバリデーションが発展んしたため、サーバ側が親切なエラーメッセージ表示を表示しなくても良くなっています。**サーバはデータの整合性のみに注力し、親切なエラーメッセージの表示はクライアントサイドに任せるというものも出てきています**。ブラウザのバリデーションを迂回し、悪質なリクエストを投げてきたユーザにわざわざ親切なメッセージを返す必要はないという考え方です。
...
* デフォルトではバリデーションはフォーム送信直前に実行され、エラーのあったフィールドまでスクロールした上でフォーカスが移動し、エラーメッセージが表示されます。これだけでは不十分な場合はJavaScript/CSSを書くことでエラー表示タイミングや表示方法をカスタマイズできます。(下記参照)
...
* `action`および`target`を使い、バリデーション実行のタイミングとエラーの表示箇所を指定しています。
...
* `:user-invalid`はブラウザネイティブだがかなり高度な制御をしており、ユーザがバリデーションエラーを実際に起こしてしまうまではエラーを表示しません。最初から鬱陶しいエラーメッセージを表示しないようになっています。
...
* 上記はStimulus controller (ValidatorController)のコードです。
...
* バリデーションの冗長なコードをまとめたJavaScriptクラスです。
