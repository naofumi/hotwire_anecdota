...
前のページの繰り返しも含みますが、まず考え方を解説します。

1. **モーダルの内容はサーバから非同期通信で取得します**。したがってTurboを使用します
...
3. Stimulusを使う場合はステートの持ち方も検討します
   1. ステートはモーダルの開閉状態だけですので、HTML要素のCSSクラスにステートを持たせる簡易方でも可能に思えます
   2. しかし下記の理由により、Stimulus Valuesステートを使うことにします
      1. 表示を切り替える箇所はモーダル「枠」だけでなく、 背景画面を覆う黒い幕も表示しないといけません
      2. さらに背景画面はキーボード等で操作できないようにしなければなりません。[HTMLの`inert`属性](https://developer.mozilla.org/ja/docs/Web/API/HTMLElement/inert)を使います
      3. **少なくとも上記の３つのHTML要素を制御する必要があります**ので、直接CSSクラスを操作すると煩雑になります。Stimulus Valuesを使って整理しやすく保った方が良いでしょう
      4. 加えてStimulus Valuesを使うと、HTMLの`data-*-value`属性を外部から変更してモーダルの開閉状態を制御でき、便利です
4. モーダルはHTML `body`要素の直下ぐらいに配置するの一般的です（一方で黒い幕を使わないポップアップダイアログであれば、モーダルのHTMLはボタンの近くに用意することが多いです）。今回もそのようにします
   1. モーダルを表示するためのActionボタン（例えばTodoを表示している行）と、モーダル自身がDOM上で完全に別々になります
      1. モーダルおよびActionボタン双方を制御するStimulus controllerを作ってしまうと、ページ全体を覆うほどの大きさになってしまいます。絶対に悪いわけではありませんが、わかりにくくなるのが気がかりです
   2. これを回避するためには**Stimulus controller間通信機能**を使います
