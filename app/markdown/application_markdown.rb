...
class ApplicationMarkdown < MarkdownRails::Renderer::Rails
  ...
  def block_code(code, language)
    ...
    content_tag(:div, class: "my-2") do
      safe_join([
                  content_tag(:div, class: "px-2 border rounded-t text-sm border-gray-700 bg-gray-700 text-gray-100 w-fit") do
                    safe_join([
                                content_tag(:span, filename_part || language_part),
                                filename_part && link_to(github_logo, "https://github.com/naofumi/hotwire_anecdota/tree/master/#{filename_part}", target: "_blank")
                              ])
                  end,
